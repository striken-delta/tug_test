...
if sys.platform == 'linux2' or sys.platform == 'linux':
    ...
    try:
        if (sys.version_info.major == 3 and sys.version_info.minor >= 8) or (sys.version_info.major > 3):
            _galaxy_root = os.environ.get("GALAXY_GENICAM_ROOT", "")
            _sdk_root = _galaxy_root.split("GenICam")[0] if _galaxy_root else ""
            _dx_dll_path64 = os.path.join(_sdk_root, "APIDll", "Win64", "DxImageProc.dll")
            _dx_dll_path32 = os.path.join(_sdk_root, "APIDll", "Win32", "DxImageProc.dll")
            if os.path.exists(_dx_dll_path64):
                dll = WinDLL(_dx_dll_path64)
            elif os.path.exists(_dx_dll_path32):
                dll = WinDLL(_dx_dll_path32)
            else:
                dll = WinDLL('DxImageProc.dll')
        else:
            dll = WinDLL('DxImageProc.dll')
    except (OSError, FileNotFoundError) as e:
        print(f'Cannot find DxImageProc.dll: {e}')
