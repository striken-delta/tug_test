...
if sys.platform == 'linux2' or sys.platform == 'linux':
    ...
    try:
        ...
        if (sys.version_info.major == 3 and sys.version_info.minor >= 8) or (sys.version_info.major > 3):
            ...
            # Use full path to GxIAPI.dll for reliable loading on Python 3.8+
            _gxiapi_dll_path = os.path.join(GxiApi_AddPath64 if os.path.exists(GxiApi_AddPath64) else GxiApi_AddPath32, 'GxIAPI.dll')
            dll = WinDLL(_gxiapi_dll_path)
        else:
            dll = WinDLL('GxIAPI.dll')
    except (OSError, KeyError, FileNotFoundError) as e:
        print(f'Cannot find GxIAPI.dll: {e}')
